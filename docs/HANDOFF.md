...
**⑬ 재료비가 0 으로 계산될 제품은 마감 전에 경고한다**
`findMissingCostBasis()`. `confirm_period()` 의 재료비는 *실적 있으면 `material_usages`,
없으면 `recipe_items` × 생산량*인데 **둘 다 0 이면 재료비 0 원이 조용히 확정된다.**
에러도 0 행도 아니고 그냥 0 이라 화면상 구분이 안 된다.
실제 DB 가 그 상태다 — 표준 배합은 수량이 0 이거나 비어 있고(엑셀 자동 등록 제품),
지금은 모든 달에 수불자료가 있어 드러나지 않을 뿐이다. 엑셀을 올리지 않은 달을
마감하는 순간 전 제품 재료비가 0 이 된다. ⑧과 같은 이유로 막지는 않고 경고만 한다.
...
환율 산출의 **계산 원가** 는 `unit_cost` 다 — 대시보드 수익성 표의 '경영 총원가' 열과 같은 값.
`total_cost` 는 월 전체 금액이라 판매가·수량과 단위가 맞지 않는다.
