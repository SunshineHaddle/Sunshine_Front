...
2. **원가·통계 열람 권한** — `read` 정책이 전 테이블 개방이라 실무자도 원가를 읽을 수 있다. §1
3. **원자재/부자재 구분** — `materials` 에 분류 컬럼이 없어 나눠 볼 수 없다. §5
4. **연말 합산** — 월별 집계까지만 있고 연 단위가 없다. §4
5. 레시피 불러오기 — 새 제품마다 배합을 처음부터 입력해야 한다. §6
