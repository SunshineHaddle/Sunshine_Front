...
**한 아이디는 한 곳에서만 쓴다 — 나중에 온 쪽을 막는다.**
계정이 둘뿐이라 공유하기 쉬운데, 같은 달을 두 사람이 동시에 고치면 월별 입력이
upsert 라 나중에 저장한 쪽이 앞사람 것을 조용히 덮어쓴다.

판정은 `profiles.last_active_at` 하나로 한다:

| | |
|---|---|
| 화면이 열려 있는 동안 | 1분마다 `touch_last_active()` (탭이 숨겨져 있으면 보내지 않는다) |
| 로그인할 때 | 그 값이 **3분** 안이면 '사용 중' 으로 보고 거부 |
| 로그아웃할 때 | `release_session()` 으로 자리를 비운다 |

> ⚠️ **브라우저를 그냥 닫으면 3분간 아무도 못 들어온다.** `release_session()` 이
> 불리지 않기 때문이다. 그 대신 잠겨서 영영 못 들어가는 일이 없다 — 유효시간이
> 곧 자물쇠를 푸는 장치다. 3분은 하트비트(1분)보다 넉넉해야 잠깐 끊긴 네트워크
> 때문에 멀쩡히 쓰는 사람이 밀려나지 않는다.

거부할 때의 `signOut` 은 반드시 `{ scope: 'local' }` 이어야 한다 —
기본값(global)은 **지금 쓰고 있는 사람의 세션까지** 끊는다.
