...
1단계의 월 칩은 **선택한 연도의 1~12월**을 항상 생성하고, DB 회차는 상태(마감/작성중)만 입힌다.
연도는 헤더의 드롭다운에서 고른다 (올해 ±5년, 더 오래된 회차가 있으면 그 해까지 넓힌다).
회차가 있는 달만 그리면 입력한 달만 띄엄띄엄 떠서 줄 수가 들쭉날쭉했다.
칸이 좁아 상태 글자는 넣지 못하고 점 색·테두리로 구분한다 (툴팁으로 보완).

**아직 오지 않은 달은 누를 수 없다.** 누르면 그 달 회차가 만들어져 빈 미래 회차가 쌓인다.
연도 드롭다운도 올해까지만 고를 수 있다 — 연도를 바꾸면 그 달로 바로 이동하므로 같은 구멍이다.
단, 이미 회차가 있는 달·해는 열어둔다 (시드나 다른 경로로 만들어졌을 수 있다).
...
감지 경로가 셋이다. 하나만으로는 새는 구멍이 있다:

| 경로 | 잡는 상황 |
|---|---|
| `onAuthStateChange` | 로그아웃 · 다른 기기에서의 signOut · 갱신 실패 |
| `visibilitychange`·`focus` | 절전·탭 방치 후 복귀. 그동안 갱신 타이머가 멈춰 있었다 |
| `fetch` 의 401 (`setAuthFailureHandler`) | 만료 토큰으로 이미 나간 요청 |

**계정 비활성화도 같은 자리에서 본다.** `is_active` 는 `profiles` 컬럼일 뿐이라
Auth 는 모른다 — 관리자가 계정을 꺼도 **이미 로그인한 사람의 토큰은 계속 갱신된다.**
쓰기는 RLS 가 막지만(`my_role()` 이 `is_active` 를 본다) 읽기 정책은 `using (true)` 라
자료를 계속 볼 수 있었다. 화면 복귀 확인에서 `is_active` 까지 보고 로그인 화면으로 돌린다.
프로필 조회가 실패하면(네트워크) 끊지 않는다 — 행이 있는데 `false` 인 경우만 막는다.

`fetch` 가로채기는 `supabase.ts` 에 있다 — API 함수 수십 개를 손대지 않으려고
클라이언트 한 곳에서 잡는다. 로그인 실패는 400 이라 여기 걸리지 않는다.
...
**한 아이디는 한 곳에서만 쓴다.** `signIn()` 이 성공하면 `signOut({ scope: 'others' })`
로 먼저 들어와 있던 세션을 끊는다. 계정이 둘뿐이라 공유하기 쉬운데, 같은 달을 두 사람이
동시에 고치면 월별 입력이 upsert 라 나중에 저장한 쪽이 앞사람 것을 덮어쓴다.
끊긴 쪽은 다음 토큰 갱신에서 막히고 `onSessionLost` 가 로그인 화면으로 돌린다 —
접속 토큰이 남은 동안(최대 1시간)은 조회가 되므로 즉시 차단은 아니다.
